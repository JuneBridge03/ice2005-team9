...
module controller (
    ...
    reg [6:0] wait_cnt; // PE 연산 시간(16클럭)을 속으로 세기 위한 스톱워치
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (current_state == WAIT_PE && wait_cnt < 7'd48) begin
    ...
    always @(*) begin
        case(current_state)
            ...
            WAIT_PE: begin
                //  핵심: 외부 신호를 받을 필요 없이 속으로 70클럭을 셉니다!
                if (wait_cnt >= 7'd48) next_state = COMPUTE;
